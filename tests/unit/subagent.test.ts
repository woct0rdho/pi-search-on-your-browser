...
test("resolveConfig: reads browser.proxy from the config file", () => {
  withTempConfigDir((dir) => {
    writeFileSync(
      join(dir, "pi-search-on-your-browser.json"),
...
test("resolveConfig: falls back to PI_SEARCH_PROXY, then direct", () => {
  withTempConfigDir((dir) => {
    writeFileSync(join(dir, "pi-search-on-your-browser.json"), JSON.stringify({ enabled: true }));
...
test("resolveConfig: tolerates a malformed config file and an invalid proxy", () => {
  withTempConfigDir((dir) => {
    writeFileSync(join(dir, "pi-search-on-your-browser.json"), "{ this is not json");
...
test("saveConfigFile: persists the proxy under browser.proxy", () => {
  withTempConfigDir((dir) => {
    ...
    try {
      ...
      saveConfigFile();
      const raw = JSON.parse(readFileSync(join(dir, "pi-search-on-your-browser.json"), "utf-8"));
